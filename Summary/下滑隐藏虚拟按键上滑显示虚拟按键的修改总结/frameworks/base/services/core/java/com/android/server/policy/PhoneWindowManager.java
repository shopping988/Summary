...
import android.app.ActivityManager.RunningAppProcessInfo;//add by zhouguo at 20170704
import android.app.ActivityManager.RunningTaskInfo;//add by zhouguo at 20170704
import android.content.res.TypedArray;
import com.reacheng.featureoption.RcFeatureOption; //add by zhouguo for Hall at 20170718
...
public class PhoneWindowManager implements WindowManagerPolicy {
    ...
    private void interceptPowerKeyDown(KeyEvent event, boolean interactive) {
        ...
        //AddMe by zhouguo 20170704 for send recentKey Broadcast begin
        if (isKeysTest() || mAllKeyDisable) {
            sendKey(KeyEvent.KEYCODE_POWER);
            return ;
        }
        //AddMe by zhouguo 20170704 for send recentKey Broadcast end
    ...
    public void init(Context context, IWindowManager windowManager,
        ...
        IntentFilter changeAllKeyStateFilter = new IntentFilter();
        changeAllKeyStateFilter.addAction("android.app.action.HOME_RECENT_KEY_ENABLE");
        changeAllKeyStateFilter.addAction("android.app.action.HOME_RECENT_KEY_DISABLE");
        context.registerReceiver(mChangeAllKeyState, changeAllKeyStateFilter);
        ...
        mSystemGestures = new SystemGesturesPointerEventListener(context,
                new SystemGesturesPointerEventListener.Callbacks() {
                    ...
                    public void onSwipeFromBottom() {
                        Log.v("hide_nav", "onSwipeFromBottom");
                        showNavBar();  
                        /// M: Disable gesture in immersive mode. {@
                        if (isGestureIsolated()) {
                            return;
                    ...
                    public void onSwipeFromRight() {
                        Log.v("hide_nav", "onSwipeFromRight");
                        showNavBar();  
                        /// M: Disable gesture in immersive mode. {@
                        if (isGestureIsolated()) {
                            return;
    ...
    void sendKey(int key) {
        Intent intent = null;
        android.util.Log.d("MidTest", ">>>>>>>>>> key : "+key);
        switch (key) {
            ...
            case KeyEvent.KEYCODE_ENDCALL:
                intent = new Intent("com.reallytek.wg.EarphoneTestActivity");
                break;
            ...
            case KeyEvent.KEYCODE_MENU:
                intent = new Intent("com.reallytek.wg.KeyTestActivity_menu");
                break;
            default:
                break;
        }
    ...
    boolean isKeysTest() {
        ...
        android.util.Log.d("MidTest", "key filter isKeysTest>>>>>>> comName:"+comName.getClassName());
        if (("com.reallytek.wg.KeyTestActivity".equals(comName.getClassName())) || 
			("com.mediatek.FactoryTest.KeyTest".equals(comName.getClassName()))) {
            return true;
        } else {
            return false;
        }
    ...
    boolean isMidTest() {
        ...
        List<RunningTaskInfo> runningTasks = am.getRunningTasks(1);
        ComponentName comName = runningTasks.get(0).topActivity;
        android.util.Log.d("MidTest", "key filter isMidTest>>>>>>>PackageName:"+comName.getPackageName());
        if (("com.reallytek.wg".equals(comName.getPackageName())) || 
			("com.mediatek.FactoryTest".equals(comName.getPackageName()))){
            return true;
        } else {
            return false;
        }
	...
	//fixMe add by zhouguo 20170718 for Hall start
    boolean isHallTest() {
        ActivityManager am= (ActivityManager) mContext.getSystemService(Context.ACTIVITY_SERVICE);
        List<RunningTaskInfo> runningTasks = am.getRunningTasks(1);
        ComponentName comName = runningTasks.get(0).topActivity;
        android.util.Log.d("test", ">>>>>>>>>>>>>>>>> comName:"+comName.getClassName());
        if (("com.reallytek.wg.HallTestActivity".equals(comName.getClassName())) || 
			("com.mediatek.FactoryTest.KeyTest".equals(comName.getClassName()))) {
			android.util.Log.d("test", ">>>>>>>>>>>>>>>>>isHallTest=true ");
            return true;
        } else {
        	android.util.Log.d("test", ">>>>>>>>>>>>>>>>>isHallTest=false ");
            return false;
        }
    }
    //fixMe add by zhouguo 20170718 for Hall end
    ...
    public int interceptKeyBeforeQueueing(KeyEvent event, int policyFlags) {
        ...
        switch (keyCode) {
            ...
            case KeyEvent.KEYCODE_HALL_OPEN:
                if(!RcFeatureOption.RC_HALL_SUPPORT){
                    break;
                }
                TelecomManager iTelecomManager = getTelecommService();
                if (isHallTest()){
                    sendKey(KeyEvent.KEYCODE_F11);  //luwl
                    break;
                }else if(iTelecomManager != null && (iTelecomManager.isRinging() || iTelecomManager.isInCall())){
                    break;
                }else{
                    if(down){
                        if(keyCode == KeyEvent.KEYCODE_HALL_CLOSE){
                            mPowerManager.goToSleep(event.getEventTime());
                        }else{
                            wakeUp(event.getEventTime(), mAllowTheaterModeWakeFromKey,"android.policy:KEY");
                        }
    ...
    private boolean mAllKeyDisable = false;
    private BroadcastReceiver mChangeAllKeyState = new BroadcastReceiver() {
        @Override
        public void onReceive(Context context, Intent intent) {
            Log.v(TAG, "mChangeAllKeyState : " + intent);
            if ("android.app.action.HOME_RECENT_KEY_ENABLE".equals(intent.getAction())) {
                mAllKeyDisable = false;
            } else if ("android.app.action.HOME_RECENT_KEY_DISABLE".equals(intent.getAction())) {
                mAllKeyDisable = true;
            }
        }
    };
    ...
    public int rotationForOrientationLw(int orientation, int lastRotation) {
        ...
        if (mForceDefaultOrientation) {
            return Surface.ROTATION_90; // modify for ratate by wangxueping 
        }
    ...
    private void showNavBar() {
        ...
        Intent intent = new Intent("com.huaying.nav_bar_control");
