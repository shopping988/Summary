...
int CommandListener::TetherCmd::runCommand(SocketClient *cli,
    ...
    if (!strcmp(argv[1], "stop")) {
        ...
        if (!strcmp(argv[1], "interface") && !strcmp(argv[2], "list")) {
            ...
            NetAddressCollection *dlist = gCtls->tetherCtrl.getDnsForwarders();
            NetAddressCollection::iterator it;
            for (it = dlist->begin(); it != dlist->end(); ++it) {
                cli->sendMsg(ResponseCode::TetherDnsFwdTgtListResult, inet_ntoa(*it), false);
            }

            NetAddressV6Collection *dlistv6 = gCtls->tetherCtrl.getDnsForwardersV6();
...
int CommandListener::FirewallCmd::runCommand(SocketClient *cli, int argc,
   ...
   if(!strcmp(argv[1], "set_volte_nsiot_firewall")){
    ...
    // net whitelist
    if(!strcmp(argv[1], "ipt_clear")) {
        int res = gCtls->firewallCtrl.iptClear();
        return sendGenericOkFail(cli, res);
    }
    
    if(!strcmp(argv[1], "ipt_disableall")) {
        int res = gCtls->firewallCtrl.iptDisableAll();
        return sendGenericOkFail(cli, res);
	}
    
    if(!strcmp(argv[1], "ipt_start")) {
        int res = gCtls->firewallCtrl.iptStart();
        return sendGenericOkFail(cli, res);
    }
    
    if(!strcmp(argv[1], "ipt_stop")) {
        int res = gCtls->firewallCtrl.iptStop();
        return sendGenericOkFail(cli, res);
	}
    ...
    if(!strcmp(argv[1], "ipt_blacklist")) {
        if (argc < 3) {
            cli->sendMsg(ResponseCode::CommandSyntaxError,
                    "Missing argument",
                    false);
        ...
        const char* addr = argv[2];
        ALOGD("wzb ipt_blacklist addr=%s\n",addr);
        int res = gCtls->firewallCtrl.iptBlackList(addr);
        return sendGenericOkFail(cli, res);
    }
    ...
    if(!strcmp(argv[1], "enable_dns_port")) {
        ...
        int res = 0;
        res |= gCtls->firewallCtrl.enableDNSPort(PROTOCOL_UDP, port);
        res |= gCtls->firewallCtrl.enableDNSPort(PROTOCOL_TCP, port);
        return sendGenericOkFail(cli, res);
    }
    ...
    if(!strcmp(argv[1], "enable_url")) {
        ...
        const char* addr = argv[2];
        int res = gCtls->firewallCtrl.enableUrl(addr);
        //int res = gCtls->firewallCtrl.setEgressSourceRule(addr);
