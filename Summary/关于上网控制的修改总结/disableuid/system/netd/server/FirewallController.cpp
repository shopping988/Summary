...
// net whitelist
char FirewallController::temp_buff[1024*1024] = {0};
...
// net whitelist
int FirewallController::iptClear(){
    ALOGD("wzb FirewallController::iptClear\n");
...
int FirewallController::iptDisableAll(){
    ALOGD("wzb FirewallController::iptDisableAll\n");
    int res=0;
    res |= execIptables(V4V6, "-F", "OUTPUT", NULL);
    res |= execIptables(V4V6, "-A", "OUTPUT","-p","tcp","-m","string","--string","Host:","--algo","bm","-j","MARK","--set-mark","1" ,NULL);
    res |= execIptables(V4V6, "-A", "OUTPUT","-p","tcp","-m","mark","--mark","1","-m","string","--string","00000000","--algo","bm","-j","ACCEPT" ,NULL);
    res |= execIptables(V4V6, "-A", "OUTPUT","-p","tcp","-m","mark","--mark","1","-j","REJECT", NULL);

    return res;
}

int FirewallController::iptStart(){
    int res=0;
    res |= execIptables(V4V6, "-A", "OUTPUT","-p","tcp","-m","string","--string","Host:","--algo","bm","-j","MARK","--set-mark","1" ,NULL);
    return res;
}

int FirewallController::iptStop(){
    int res=0;
    res |= execIptables(V4V6, "-A", "OUTPUT","-p","tcp","-m","mark","--mark","1","-j","REJECT", NULL);
    return res;
}
...
int FirewallController::iptWhiteList(const char *addr){
    ...
    //char temp[1024*1024]={0}; //bug stack limit
...
int FirewallController::iptBlackList(const char *addr){
    ALOGD("wzb FirewallController::iptBlackList:%s\n",addr);
    //char temp[1024*1024]={0}; //bug stack limit
    strcpy(temp_buff,addr);
    ALOGD("wzb  temp=%s\n",temp_buff);
    char *p;
    ...
    res |= execIptables(V4V6, "-F", "OUTPUT", NULL);
    while(p!=NULL){
        ALOGD("wzb blacklist=%s\n",p);
        //res |= execIptables(V4V6, "-A", "OUTPUT","-p","tcp","-m","mark","--mark","1","-m","string","--string",p,"--algo","bm","-j","ACCEPT" ,NULL);
        //res |= execIptables(V4V6, "-A", "OUTPUT","-p","tcp","-m","mark","--mark","1","-j","REJECT", NULL);
        //res |= execIptables(V4V6, "-A", "INPUT","-m","string","--string",p,"--algo","bm","-j","REJECT" ,NULL);
        p = strtok(NULL, split);
    }
    res |= execIptables(V4V6, "-A", "OUTPUT","-j","ACCEPT", NULL);
    return res;
}
...
int FirewallController::enableDNSPort(int protocol,int port) {
    ...
    res |= execIptables(V4, "-I", LOCAL_FORWARD, "-p", protocolStr, "--sport", portStr, "-j", "ACCEPT", NULL);
    res |= execIptables(V4, "-I", LOCAL_FORWARD, "-p", protocolStr, "--dport", portStr, "-j", "ACCEPT", NULL);
    res |= execIptables(V4, "-I", LOCAL_FORWARD, "-p", protocolStr, "--sport", "80", "-j", "ACCEPT", NULL);
    res |= execIptables(V4, "-I", LOCAL_FORWARD, "-p", protocolStr, "--dport", "80", "-j", "ACCEPT", NULL);
    return res;
}
...
int FirewallController::disableUrl(const char* addr) {
    ...
    int res = 0;
    //if(1) return res;
    res |= execIptables(V4, "-I", LOCAL_INPUT, "-s", addr, "-j", "REJECT", NULL);
    res |= execIptables(V4, "-I", LOCAL_FORWARD, "-s", addr,"-j", "REJECT", NULL);
    res |= execIptables(V4, "-I", LOCAL_OUTPUT, "-d", addr, "-j", "REJECT", NULL);
    res |= execIptables(V4, "-I", LOCAL_FORWARD, "-d", addr,"-j", "REJECT", NULL);
    res |= execIptables(V4, "-t", "mangle", "-I", LOCAL_MANGLE_POSTROUTING, "-s", addr,"-j", "REJECT", NULL);
    res |= execIptables(V4, "-t", "mangle", "-I", LOCAL_MANGLE_POSTROUTING, "-d", addr,"-j", "REJECT", NULL);
    //res |= execIptables(V4, "-I", LOCAL_INPUT, "-s", "120.76.47.120", "-j", "ACCEPT", NULL);
    //res |= execIptables(V4, "-I", LOCAL_OUTPUT, "-d", "120.76.47.120", "-j", "ACCEPT", NULL);
    //res |= execIptables(V4, "-t", "mangle", "-I", LOCAL_MANGLE_POSTROUTING, "-s", "120.76.47.120","-j", "ACCEPT", NULL);
    //res |= execIptables(V4, "-t", "mangle", "-I", LOCAL_MANGLE_POSTROUTING, "-d", "120.76.47.120","-j", "ACCEPT", NULL);
